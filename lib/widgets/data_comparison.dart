...
Widget buildDataComparison(
  ...
  switch (selectedViewType) {
    ...
    case ViewType.oneRepMax:
      ...
      title = 'Максимум одно повторение';
  }



  title += selectedGroupBy == GroupBy.muscleGroup ? ' по группам мышц' : ' с помощью упражнений';
  ...
  switch (timeFrame) {
    ...
    case TimeFrame.lastMonth:
      for (int i = 29; i >= 0; i--) {
        ...
        final weekKey = 'Неделя $weekNumber';
...
Map<String, Map<String, num>> convertOneRepMaxToMuscleGroup(Map<String, Map<String, double>> oneRepMaxByExercise) {
  ...
  oneRepMaxByExercise.forEach((exercise, dateMap) {
    String muscleGroup = ExerciseInstance.exerciseDetails[exercise.toLowerCase()]?['мышечная группа'] ?? 'неизвестный';
